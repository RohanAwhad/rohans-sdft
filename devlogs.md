...
- **verify_adapter gate (spec step 2)**: HF base+peft adapter logits vs Megatron+LoRA logits — top-20 agreement 0.9727 (2101/2160), mean logit diff 0.0973 → PASS (threshold 0.95; 1.0 was too strict, fp32-export vs bf16 precision flips near-ties; a wrong q/k/v split would drop to ~30%). Script removed after the one-time check (commit 0af7aab; restore via `git show 0af7aab^:megatron_trainer/verify_adapter.py`).
...
- verify_adapter needed `TRAIN_DATA_PATH` set (config.py import side effect) when run standalone — moot now that it's removed (0af7aab), noted for any future restore.
...
- **Implementation + LoRA pivot (this session, branch ra/grpo-live)**: built `LOSS_TYPE=grpo` per this spec (see below), banning LoRA initially per direct instruction. Smoke-tested on gpt-oss-20b: found and fixed a real perf bug (LM-head GEMM called per row-chunk instead of once — `chunked_head.py`), then hit a genuine CUDA OOM at the first `optimizer.step()` (FSDP 2-way shard of a 20B model leaves no room for AdamW state on an 80GB H100 — unrelated to GRPO, would hit SDFT too at this trainer count). Reversed the LoRA ban after being pointed at PR #23 (issue #22 fix) — LoRA sidesteps the optimizer-state ceiling entirely. Found + fixed one more bug (`_step_tail`'s lora branch synced to the logprob server unconditionally, crashing when `USE_LOGPROB_SERVER=False`). `TRAIN_MODE=lora` is now the v1 GRPO default.
