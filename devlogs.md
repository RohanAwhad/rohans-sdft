...
## 2025-07-14 — Task 3: Adapter SDFT on Synthetic Algebra

### Experiment

Train an adapter model via SDFT that checks an API model's arithmetic answers and provides correction guidance. The core asymmetry: the teacher sees explicit symbol→operation mapping (θ=+, α=-, γ=×, β=÷), the student only gets a vague hint ("each maps to one operation"). The KL signal teaches the student to match the teacher's distribution on when to CORRECT vs. when to give guidance.

**Flow**: User → API → Adapter → API → User
- If API answer is correct: adapter outputs `\boxed{CORRECT}` (pass-through)
- If API answer is wrong: adapter outputs natural language guidance (sent back to API for retry)

### Dataset

- Generated by `synthetic_algebra_dataset.py` with `--maas --api-model`
- Format: `{prompt: [...], user_response: {...}}` (same MaaS pattern as existing trainer)
- `prompt` = student view (vague symbols + expression + API answer + few-shot examples)
- `user_response` = teacher privilege (mapping + correct answer)
- API answers from Claude Haiku 4.5 (not synthetic)
- Train: 1280 samples (640 custom, 640 standard), Test: 320
- Claude Haiku API accuracy: 56.4% overall (36.7% custom, 76.1% standard)
- Location on node: `~/rawhad/rohans-sdft/data/` (rh-h100-05)

### Key design decisions

- **Full finetuning** (no LoRA) — same setup as existing MaaS SDFT
- **Nemotron-style thinking client**: two-step vLLM generation with 128-token reasoning budget, 256-token total ceiling
- **Adapter prompt is raw text** (no chat template) — `task_3/collator.py` handles formatting
- **Config**: `ADAPTER_MODE=1`, `REASONING_BUDGET=128`, `GEN_MAX_NEW_TOKENS=256`, LR=5e-5, EMA=0.05

### Baselines (Qwen3-8B, untrained, n=50 test samples)

| Metric | Value |
|--------|-------|
| API accuracy (custom) | ~5% (cannot determine symbol mapping) |
| API accuracy (standard) | ~100% (with enough tokens, clean prompt) |
| Adapter binary accuracy | 74.0% |
| Adapter precision | 100.0% |
| Adapter recall | 23.5% |
| Confusion matrix | TP=4, TN=33, FP=0, FN=13 |

Qwen3-8B defaults to `\boxed{CORRECT}` — high precision (only flags when certain) but low recall (misses 76% of wrong API answers). The model doesn't understand the task yet; it hallucinates and loops the few-shot pattern.

### Claude Haiku API accuracy (on full test set)
...
Claude tries to reason through the mapping but with only 1 expression can't determine the mapping uniquely. Gets some right by chance/trial.

### Targets (from api-adapter-ak GRPO)
...
- `synthetic_algebra_dataset.py` — MaaS format + Claude API answer generation
- `task_3/collator.py` — AdapterCollator (no chat template)
- `task_3/eval.py` — Mode 1 (binary classification) + Mode 2 (two-pass guidance)
- `task_3/launch.sh` — GPU assignment + adapter env vars
- `task_3/baseline.py` — API + adapter baseline evaluation
- `train_dir/src/vllm_utils.py` — `vllm_generate_with_thinking()` (Nemotron budget pattern)
- `train_dir/src/config.py` — `ADAPTER_MODE`, `REASONING_BUDGET`
- `train_dir/src/trainer.py` — Conditional adapter collator + thinking generation

### Status

- [x] Dataset generated on rh-h100-05 (1280 train / 320 test, Claude Haiku API answers)
- [x] AdapterCollator + thinking client implemented
- [x] Baselines measured (n=50)
- [ ] Full 320-sample baseline run (failed due to HF auth, need retry)
- [ ] Train on rh-h100-05
- [ ] Eval checkpoint against baselines
