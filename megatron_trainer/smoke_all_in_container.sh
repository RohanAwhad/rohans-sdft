...
LOSS_TYPE=${LOSS_TYPE:-sdft}
GRPO_GROUPS=${GRPO_GROUPS:-8}
if [ "$LOSS_TYPE" = "grpo" ]; then
    SMOKE_GRAD_ACCUM=${GRAD_ACCUM_STEPS:-$((NUM_TRAINERS * GRPO_GROUPS))}
    SMOKE_HINDSIGHT=${HINDSIGHT_FIELD:-online_feedback}
else
    SMOKE_GRAD_ACCUM=${GRAD_ACCUM_STEPS:-$((NUM_TRAINERS * 2))}
    SMOKE_HINDSIGHT=${HINDSIGHT_FIELD:-enriched_user_response}
fi
...
TMPDIR=/mnt/nvme0n1/podman_tmp podman run --rm \
    ...
    -e NUM_EPOCHS=1 \
    -e GRAD_ACCUM_STEPS="$SMOKE_GRAD_ACCUM" \
    -e LOSS_TYPE="$LOSS_TYPE" \
    -e GRPO_GROUPS="$GRPO_GROUPS" \
    ...
    -e HINDSIGHT_FIELD="$SMOKE_HINDSIGHT" \
