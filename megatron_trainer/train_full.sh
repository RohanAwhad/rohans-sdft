...
for var in WANDB_MODE WANDB_BASE_URL WANDB_ENTITY WANDB_API_KEY \
           ...
           PYTORCH_CUDA_ALLOC_CONF TRAINER_SEED VLLM_SEED LOG_DIR NCCL_TIMEOUT_SEC \
           LOSS_TYPE GRPO_GROUPS GRPO_ADV GRPO_CLIP_LOW GRPO_CLIP_HIGH \
           GRPO_OLD_LOGPS GRPO_IS_C_MAX GRPO_KL_COEF GRPO_FILTER_GROUPS \
           GRPO_MAX_GEN_BATCHES \
           GRPO_LR GRPO_LR_WARMUP_STEPS GRPO_GRAD_CLIP GRPO_MASK_TRUNCATED; do
    if [ -n "${!var:-}" ]; then
